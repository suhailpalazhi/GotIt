document.addEventListener('DOMContentLoaded', function() {
    ...
    function animateCSS(element, animation, prefix = 'animate__') {
      // Remove any existing Animate.css classes
      element.classList.remove(...Array.from(element.classList).filter(c => c.startsWith(prefix)));
      // Add Animate.css classes
      element.classList.add(`${prefix}animated`, `${prefix}${animation}`);
      // Remove classes after animation ends (so it can replay)
    ...
    // Map section IDs to Animate.css animation names (no prefix)
    const sectionAnimations = {
        '#hero': 'fadeIn',
        '#why-choose': 'fadeInLeft',
        '#subjects': 'fadeInUp',
        '#how-it-works': 'fadeInRight',
        '#schedule': 'fadeInUp',
        '#mentors': 'fadeInLeft',
        '#resources': 'fadeInRight',
        '#testimonials': 'fadeInUp',
        '#registration': 'fadeIn',
        '#contact': 'fadeInUp'
    };
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const animClass = sectionAnimations[`#${entry.target.id}`];
                if (animClass) {
                    animateCSS(entry.target, animClass);
                }
            }
        });
    }, observerOptions);
